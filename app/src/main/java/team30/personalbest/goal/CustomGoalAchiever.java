...
public class CustomGoalAchiever implements GoalAchiever
{
	...
	public void startAchievingGoal()
	{
		...
		if (this.goal == null)
			throw new IllegalStateException("Missing step goal");
	...
	{
		if (this.running)
			throw new IllegalStateException("Cannot change step goal while running");
		this.goal = goal;
		return this;
	}
		...
		protected Void doInBackground(Void... voids)
		{
			try
			{
				while (this.currentSteps < this.goalSteps)
				{
					...
					this.iGoalService.getGoalSnapshot().onResult(new Consumer<IGoalSnapshot>() {
						@Override
						public void accept(IGoalSnapshot iGoalSnapshot)
						{
							GoalChecker.this.goalSteps = iGoalSnapshot.getGoalValue();
						}
					});
